 ...
 *
 * Пример организации списка через контейнер
 *
 *
 */
...
class list_A
{
    // Создадим контейнер для объекта типа А
    struct A_unit
    {
        A obj; // Объект который будет хранится в списке
        A_unit *next; // Ссылка на следующий элемент
        A_unit *prior; // Ссылка на предыдущий элемент
    };
...
void list_A::edit(const int num_old, const int new_num)
{
    ...
    if (num)
        num->obj.set_n(new_num);
    else
        std::wcout << L"Элемент не найден!" << std::endl;
}

list_A::A_unit* list_A::find_l(const int key) const
{
    for (A_unit* current = first;
         current;
         current = current->next)
        if (current->obj.get_n() == key)
            return current;
    return 0;
}
...
void list_A::printAll() const
{
    for (A_unit* current = first;
         current;
         current = current->next)
        std::wcout << current->obj.get_n() << std::endl;
}
